...
`behavior.one_on_one_rules` remains **partial**, not verified. Traveling, first-contact/low-ball recovery, exact `$077D` pickup limits, player priority, the `$C12D` cooldown, shooter-action exclusion, side-change shot-clock handling, and the outer dead-ball response are implemented. The meanings and scene sources for global `$FFEB/$FFE2/$FFE7` locks, collision penalties, steals, blocks, goaltending, shot contests, exact control timing, and full contact outcomes are still absent or generic. The possession coverage must not be read as completion of the full rules milestone.

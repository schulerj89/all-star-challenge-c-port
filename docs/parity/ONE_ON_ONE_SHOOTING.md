...
| `$7F37` | Chooses release offsets by action/facing while held and by shot phase/variant during the jump; phase 3 leaves the origin unchanged. | `allstar_one_on_one_rom_release_offset` contains the exact signed table and One-on-One launches consume its phase-two/class-zero result. |
...
| Fixed `$2AE2/$2B07/$2B88` | When the ball is loose and low enough, tests each eligible player and awards possession, recording whether the side changed. | Native recovery calls the recovered collision helper and resets the shot clock only when the possession side changes; ROM cooldown/flight gates remain incomplete. |
...
- `$7F37`'s signed offsets are represented, but native roster data does not expose player field `+$16`, so the scene currently uses animation class zero;

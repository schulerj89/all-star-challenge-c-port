...
| Made basket | `$1E0E` begins a score presentation instead of immediately resetting play. `$1F26` dispatches net command `$08` with the first bend at `+20`; `$1F23` commits the score and dispatches multi-step command `$05` at `+65`. |
| Post-score transition | Preserve the cartridge's two counted holds, four-step `$27C7/$27EA` fade-out, `$20F7` possession rebuild, reverse `$27CC/$27EA` fade-in, and final counted hold. `$702D` runs with `$FFEB=1` at ROM frame 254; OBJ display and playable input return at ROM frame 258. The native scene intentionally consumes this presentation timeline at 3× speed, reducing the wall-clock wait from about 4.3 to 1.43 seconds without changing shot flight. |
...
| `$7F37` then `$6F2A` | `$7F37` chooses shot gather/release offsets and record-coupled height; display frames `$00/$0C` clear the separate ball because the player frame contains it. During ordinary dribbling, later `$6F2A` is the final action/direction/record placement. | `allstar_one_on_one_rom_release_offset`, `allstar_one_on_one_rom_release_height`, and `allstar_one_on_one_rom_shot_release_height` reproduce launch coordinates; `allstar_renderer_rom_dribble_ball_6f2a` supplies ordinary live held-ball presentation. |
...
| Bank 1 `$78E9->$794B/$796C`, then `$7C58` | A changed-owner recovery sets `$FFD1`. It persists inside the central take-back region, clears only after the held ball moves outside, and prevents a launch while set. | `allstar_one_on_one_rom_take_back_cleared_78e9` uses exact ball coordinates; the scene applies one shared `take_back_required` state to human and CPU shots. |
| Fixed `$1E0E->$1F33->$1ECC` | Starts the made-basket effect and writes bend/deep/bend/rest net frames at `+20/+35/+50/+65`. `$1F26` selects command `$08` at `+20`; `$1F23/$1F06` selects command `$05` with score commit at `+65`. | The net helper/renderer use the extracted 17-tile stream; the scene maps `$08` to `ALLSTAR_SFX_SWISH` and `$05` to the separate `ALLSTAR_SFX_SCORE_CHIME`. |
...
| Bank 1 `$702D->$714D->$6A8C->$6B72` | Shot action `$0A/$12` does not clear direction `+$07`; normal shot records still execute the directional movement callback before release. | The live scene preserves held direction and keeps the ROM record engine active during gather, so the player can move before the second A edge. |
| Fixed `$1E0E`, bank 1 `$7BE8`, fixed `$1E5B/$1E77` | Holds gravity for 35 frames after the make, then produces the hard `+76` and smaller `+121/+158` floor bounces before fade `+180`. | The score-ball uses the shared 8.8 integrator, exact delay, and hard-bounce latch rather than a synthetic linear descent. |
...
| Fixed `$20F7->$2197->$21C8/$21E1->$702D` | Rebuilds possession after fade-out; the new owner receives take-out target `$4C/$98`, the prior scorer `$4C/$88`, and the ball `$50/$90`. | The scene resets at frame 214 to native centers `(84,152)` and `(84,136)`, preserves the counted update at frame 254, and resumes playable inbound at frame 258. |
...
boundaries. The live native scene applies `ALLSTAR_NATIVE_SCORE_PRESENTATION_RATE=3`
as an explicit usability override, so it renders the same state sequence in
approximately 86 display frames. This is the only intentional pacing
...
- the post-score presentation runs at 3× cartridge speed in the native scene to avoid the original 4.3-second input lock;

#!/usr/bin/env python3
"""Validate and report the scoped One-on-One parity requirement manifest."""
...
MANIFEST = ROOT / "docs" / "parity" / "ONE_ON_ONE_COVERAGE.json"
...
def main() -> int:
    ...
    report = {
        ...
        "total": expected,
        "percent": percent,
        "target_min": data["metric"]["target_percent_min"],
        "target_max": data["metric"]["target_percent_max"],
        "groups": groups,
        "errors": errors,
    }
    if args.as_json:
        ...
        print(f"One-on-One parity coverage: {verified}/{expected} ({percent:.2f}%)")
    ...
    if errors:
        return 1
    if args.require_min is not None and percent < args.require_min:
        print(f"ERROR: {percent:.2f}% is below required {args.require_min:.2f}%")
        return 1
    return 0
